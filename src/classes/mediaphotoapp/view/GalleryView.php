...
class GalleryView extends MediaphotoView implements Renderer
{
    public function render(): string
    {
        ...
        $html .= "</li></ul><div><a href=$updateGallery><button>Mettre à jour la galerie</button></a><a href=$deleteGallery><button> Supprimer la Gallerie </button></a><a href=$addTags><button> Ajouter un Tag </button></a><a href=$addPicture><button>Ajouter une photo</button></a><a href=$giveAcces><button>Donner l'accès aux utilisateurs</button></a><a href=$removeAcces><button> supprimer l'accès aux utilisateurs</button></a></div></section><article>";
        ...
        foreach ($galleryPictures as $v) {
            ...
            $html .= "<a href='$addTags'><button>Ajouter un tag</button></a><a href='$deletePicTag'><button>Supprimer un tag</button></a><a href='$deletePicture'><button> Supprimer l'image </button></a></nav></aside></div>";
        }
        $html .= "</article>";
        return $html;
    }
